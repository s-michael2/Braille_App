...
void checkUpperCase(String input){
  ...
  // entire word is capitalized, motor 6 raises twice
  if (isAllCap) {
      Serial.println("is All Cap");
...
void processInput(String input) {
  ...
  for(int i=0; i<length;i++){
    ...
    if(isDigit(currentchar)){
      // character before the current character was not a digit
      if(!isDigits){
        doDigit('#'); // moves motors to signify number
        resetBrailleDisplay();
        isDigits = true; //  sets isDigits to true so that it knows this letter is a number
        isNonDigit = false;
      }
      doDigit(currentchar); // moves moter for number shown

    }
    // current character is not a number
    else{
   // last character was a digit and the current character is a letter
      if(isDigits && isalpha(currentchar)){
        doDigit('~'); // moves motor to signify letter and not a number
        resetBrailleDisplay();
      }
...
void doDigit(char input){
  switch (input) {
    case '#': // symbolize a number is coming
      ...
      raiseMotor(motor10, 90);
      break;
    case '~': // symbolize a letter after a number
      raiseMotor(motor9, 90);
      raiseMotor(motor10, 90);
      break;
    case '1':
